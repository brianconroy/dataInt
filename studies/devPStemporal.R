...
counter <- 1
for (u in U){
  locs <- simLocW(u + W, r, beta=0, seed=11)
...
for (i in 1:length(U)){
  locs_i <- locs_time[[i]]
  u_i <- U[i]
  case.data <- simConditionalGp2(cov.disc, locs_i, beta.case, alpha.case, u_i + W, seed=42)
  ctrl.data <- simConditionalGp2(cov.disc, locs_i, beta.ctrl, alpha.ctrl, u_i + W, seed=40)
